...
USE_HTTPS = getattr(settings, 'GOOGLEOAUTH_USE_HTTPS', True)
CLIENT_ID = getattr(settings, 'GOOGLEOAUTH_CLIENT_ID', None)
CLIENT_SECRET = getattr(settings, 'GOOGLEOAUTH_CLIENT_SECRET', None)
CALLBACK_DOMAIN = getattr(settings, 'GOOGLEOAUTH_CALLBACK_DOMAIN', None)
APPS_DOMAIN = getattr(settings, 'GOOGLEOAUTH_APPS_DOMAIN', None)
GET_PROFILE = getattr(settings, 'GOOGLEOAUTH_GET_PROFILE', True)
...
def generate_redirect_uri():
    ...
    path = reverse('googleoauth_callback')
...
def login(request):
    ...
    request.session['googleoauth_csrf'] = csrf_token
...
def callback(request):
    if request.GET.get('state') != request.session.get('googleoauth_csrf'):
